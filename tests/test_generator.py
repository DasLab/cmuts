...
from support import (
    Dataset, generate, md_and_nm_tags, recomputed_md_and_nm_tags, _records,
)
...
def test_header_lengths_match_the_reference(datasets, shape):
    ...
    for line in _records(data.bam, "-H"):
...
def test_the_same_seed_gives_the_same_data(tmp_path):
    ...
    assert _records(first.bam) == _records(second.bam)
...
def test_stored_length_diverges_from_aligned_span(datasets):
    ...
    for line in _records(datasets("plain").bam, "-F", "4"):
        fields = line.split("\t")
        stored = len(fields[9])
