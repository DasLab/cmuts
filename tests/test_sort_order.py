...
def test_a_coordinate_sorted_file_is_accepted(datasets, tmp_path, name):
    data = datasets(name)
    assert try_cmuts(data, tmp_path / "out.h5").returncode == 0


@pytest.mark.parametrize("name", sorted(DATASETS))
def test_a_name_sorted_file_is_refused(datasets, tmp_path, name):
    data = datasets(name)
...
@pytest.mark.parametrize("name", sorted(DATASETS))
def test_a_header_without_an_hd_line_is_refused(datasets, tmp_path, name):
    data = datasets(name)
...
@pytest.mark.parametrize("name", sorted(DATASETS))
def test_a_sort_order_that_merely_starts_the_same_is_refused(datasets, tmp_path, name):
    data = datasets(name)
...
@pytest.mark.parametrize("name", sorted(DATASETS))
def test_other_tags_on_the_hd_line_do_not_hide_the_sort_order(datasets, tmp_path, name):
    data = datasets(name)
    altered = reheadered(
        data, tmp_path,
...
@pytest.mark.parametrize("name", sorted(DATASETS))
def test_an_hd_line_without_a_sort_order_is_refused(datasets, tmp_path, name):
    data = datasets(name)
    altered = reheadered(data, tmp_path,
