...
static bool known_at(const rate_config *cfg, double evidence)
{
    return evidence > 0.0 && evidence >= cfg->min_depth;
}
...
static double rate_at(const accum *acc, size_t i)
{
    double evidence = accum_const_data(acc, ACCUM_SPANNED)[i];
    double rate     = evidence > 0.0
                    ? accum_const_data(acc, ACCUM_MUTATIONS)[i] / evidence
                    : 0.0;
                     ...
                     double *out)
{
    ...
    for (size_t i = 0; i < len; i++)
        out[i] = known_at(cfg, evidence[i]) ? rate_at(acc, i) : (double)NAN;
}
                ...
                double *out)
{
    ...
    for (size_t i = 0; i < len; i++) {
        double rate = rate_at(acc, i);

        out[i] = known_at(cfg, evidence[i])
