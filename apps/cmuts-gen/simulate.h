 ...
 *
 * Insertions and deletions are counted as events per read with their own
 * length, rather than falling out of a per-base rate. A single rate produces
 * many one and two base indels and effectively never a long one, where real
 * aligners emit both -- and it is the long insertion, which leaves a read far
...
/* Where on a reference a read falls. Drawn separately from building the read
 * so that a caller can sort placements -- eight bytes apiece -- and emit the
 * records themselves in coordinate order, rather than holding every record of
 * a reference in memory to sort afterwards. */
typedef struct {
...
/* Builds one alignment at the given placement. The CIGAR, SEQ, MD and NM are
 * derived from a single list of per-base events, so they cannot disagree with
 * one another. Returns 0, or -1 if the record could not be built. */
...
/* Builds a record aligning nowhere, which belongs to no reference. */
