 ...
 * merged.
 *
 * Records arrive grouped by reference rather than by file, so a reference is still finished
 * the moment the stream moves past it. Running the files one after another would visit each
...
/* The htslib record behind the most recent cm_bam_stream_next(), for callers that must
...
/* Answered from the first file's header, the others being held to it by the checks refseq
 * makes on each. */
...
/* The files themselves, for checks that must reach each header in turn. */
