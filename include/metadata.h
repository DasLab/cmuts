...
/* Totals belonging to the run as a whole. Unmapped reads are the only such quantity, aligning
 * to no reference and so having no row to be counted in. Everything else follows from the
 * per-reference datasets. */
